(() => {
  ...
  function buildTaskPortalChecklist(model) {
    ...
    model.taskLinks.slice(0, 8).forEach((task) => {
      items.push({
        text: `Choose task: ${task.label}`,
        target: task.element,
        kind: 'task',
      });
    });
  ...
  function buildGenericChecklist(model) {
    ...
    model.taskLinks.slice(0, 4).forEach((task) => {
      items.push({ text: `Choose task: ${task.label}`, target: task.element, kind: 'task' });
    });
  ...
  function renderSummary(model) {
    ...
    if (model.pageType === 'Task Portal') {
      summaryEl.textContent = `I found ${model.taskLinks.length} useful task link${model.taskLinks.length === 1 ? '' : 's'}. Choose the one closest to why you came here.`;
      return;
    }
